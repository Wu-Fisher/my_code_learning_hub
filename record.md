...
### 825.适龄的朋友
    ...
    本题目中y<=0.5x+7的时候不会收到x的请求，也就是说y 15岁以前同龄人也不行，而同龄的人都不会发消息的话，x年龄更大的人也一定不会发消息（这里就很密，会导致需要添加一个判断）
...
### 1713.得到子序列的最少操作次数
    ...
    如果但看LIS问题其实上面的 334 （非该题的优化解解法）更加典型， 因为那个是严格要求的数组的值的递增。
